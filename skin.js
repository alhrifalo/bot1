...
module.exports = {
    ...
    run(client, message, args) {
        ...
        if (!accountId || isNaN(amount)) {
            message.channel.send("**Ex: [.setskin ID skinId] | قم بكتابة الايدي والمبلغ من فضلك**");
            return;
        }
        ...
        db.connect(err => {
            if (err) throw err;
            const selectSql = 'SELECT skin FROM characters WHERE account = ?';
            db.query(selectSql, [accountId], (err, results) => {
                ...
                if (results.length === 0) {
                    ...
                    const oldSkin = results[0].skin;
                    const newSkin = amount;
                    const updateSql = 'UPDATE characters SET skin = ? WHERE account = ?';
                    db.query(updateSql, [newSkin, accountId], (err) => {
                        if (err) throw err;
                        ...
                        .addField("- Old skin id:", oldSkin, true)
                        .addField("- New skin id:", newSkin, true)
